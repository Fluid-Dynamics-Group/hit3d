...
subroutine data_step(itr)
    !========================
    integer*4 :: nx, ny, nz, npoints
    integer*4 :: sizes(3)
    real :: dx, dy, dz
    ...
    do k = 1, nz
        do j = 1, ny
            do i = 1, nx
                ...
                brooks_helicity = brooks_helicity + (u_vec(l)*OmgX_vec(l) + v_vec(l)*OmgY_vec(l) + w_vec(l) * OmgZ_vec(l))
